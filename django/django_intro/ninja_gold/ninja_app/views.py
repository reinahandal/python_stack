...
def money(request):
    if request.POST['activity'] == 'farm' or request.POST['activity'] == 'cave' or request.POST['activity'] == 'house':
        ...
        request.session['activity_log'].insert(0, ['You entered a '+ current_activity + ' and earned ' + str(random_number) + ' gold.', 'success'])
        ...
        request.session['current_activity'] = current_activity
        if random_number < 0:
            request.session['activity_log'].insert(0, ['You failed a quest and lost ' + str(random_number) + ' gold. Ouch.', 'lose'])
        else:
            request.session['activity_log'].insert(0, ['You entered a quest and earned ' + str(random_number) + ' gold.', 'success'])
